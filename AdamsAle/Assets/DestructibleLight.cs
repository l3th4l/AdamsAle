            ...
            if(Ent.GetComponent<HostileAI>()!= null)
            {
                ...
                if(Physics.Raycast(transform.position,(Ent.transform.position-transform.position).normalized,out _hit,RayMask))// Checks if Entities are not obstructed
                {
                    if(_hit.collider == Ent)
                        VAEnt.LI_Inc = Intensity * (Rad - Vector3.Magnitude(Ent.transform.position - transform.position))/Rad; // Increases the light intensity varible in the Entity's Camera's ViewAdjust script by the 1/distance from the light * intensity
                }
            }
        }
	}
